...
fileName = "/home/hadoop/spark_sql/parseFile.txt"
SEARCH_PATH = "serp_algo"; 
...
for line in sql_list:
	...
	while i < len(sql_string_list):
		...
		elif "instr(" in sql_string_list[i].lower():
			#add anything in front of instr to executable_sql and work with the rest
			slist = sql_string_list[i].lower().split("instr(");
			executable_sql+=slist[0] + " POSITION( ";
			subList = [];
			param = slist[1].split(",");
			if len(param) > 1:
				for x in param:
					if x.count("(") < x.count(")"):
						occur = x.count("(") + 1;
						indices = [ind.start() for ind in re.finditer(")", x)];
				...
				slist.append(param[0]);
				i+=1;
				while i < len(sql_string_list):
					if sql_string_list[i].count("(") < sql_string_list[i].count(")"):
						occur = x.count("(") + 1;
						indices = [ind.start() for ind in re.finditer(")", x)];
						part1 = string[0:indices[occur-1]];
						part2 = string[indices[occur-1]+1:];
						...
						slist.append(sql_string_list[i]);

					i+=1;
				#find the remaining param after
	...
	if executable_sql != "(" and table_name != "":
		...
		#df_list.append(sqlctx.read.format('jdbc').options(url="jdbc:redshift://test.cv741ke2k9an.us-east-1.redshift.amazonaws.com:5439/test?user=test&password=Test1234&searchpath="+SEARCH_PATH, dbtable=executable_sql));
